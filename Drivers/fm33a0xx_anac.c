...
功能：读SVDSVD内部滤波后的电压检测标志状态，在不使能数字滤波时，此信号等同于SVDO；软件避免写此寄存器。
输入: 无
输出: SET   不欠压
      RESET  欠压
*********************************/
...
输出: 电压值
*********************************/
float ANAC_ADC_VoltageCalc2(float fADCData, uint8_t Vdd)
{
	float fVolt = 0;
	
	fVolt = ANAC_ADC_VoltageCalc(fADCData);
...
输出: 电压值
*********************************/
uint32_t ANAC_ADC_VoltageCalcInt(uint32_t ADCData)
{
	uint32_t Volt = 0;
...
//		fVolt = fADCData*((const_adc_Slope/1000.0))+(float)const_adc_Offset/100.0;	
		if(ADCData <= 30)
			...
			Volt = (ADCData*const_adc_Slope/1000);
		else if(ADCData <= 500)
			Volt = (ADCData*98/100*const_adc_Slope/1000);
		else
...
输出: 电压值
*********************************/
uint32_t ANAC_ADC_VoltageCalcInt2(uint32_t ADCData, uint8_t Vdd)
{
	uint32_t Volt = 0;
	
	Volt = ANAC_ADC_VoltageCalcInt(ADCData);

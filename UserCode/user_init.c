...
void TicksDelay(uint32_t ClkNum)
{
    ...
    if(clkmode != 4)
	{
		ClkNum = ClkNum*clkmode;//适应不同主频，8M时最大定时2s，32M时最大定时0.5s
	}
	else
	{
		ClkNum = ClkNum*clkmode*30/32;//30M时不是整数倍
	}
...
void Init_IO(void)
{
	...
	OutputIO( LED1_GPIO, LED1_PIN, 0 );	 //led0

	OutputIO( LCDPOWER_GPIO, LCDPOWER_PIN, 0 );
	OutputIO( LCDCS_GPIO, LCDCS_PIN, 0 );
	OutputIO( LCDA0_GPIO, LCDA0_PIN, 0 );
	OutputIO( LCDRST_GPIO, LCDRST_PIN, 0 );
	OutputIO( LCDSCL_GPIO, LCDSCL_PIN, 0 );
	OutputIO( LCDSDA_GPIO, LCDSDA_PIN, 0 );
	OutputIO( LCDBACKLIGHT_GPIO, LCDBACKLIGHT_PIN, 0 );
